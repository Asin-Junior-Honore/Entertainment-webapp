...
const Home = ({ addbookMark, addbookMarkposter }) => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/tv/top_rated?api_key=0c1d1512d06f72a1b62a4876eff89834&page=5`
    )
    ...
    const Apidata = async () => {
      let response = await fetch(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=0c1d1512d06f72a1b62a4876eff89834&page=5`
      );
  ...
  const handleFetch = () => {
    if (value.length == 0) {
      alert("please type a movie or tv series");
    } else {
    ...
    <div className="Moviewrapper">
      <>
        ...
        <div className="recomended">
          <h2>Recommended Movies and Tv-shows</h2>
          {isloading ? (
            <HashLoader color="#c87373" />
          ) : (
